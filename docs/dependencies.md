...
## Baseline: last run 2026-07-21

**Resolved:**
- npm: all alerts cleared (postcss, picomatch x2, flatted, minimatch x2) + `npm audit` extras (ajv, brace-expansion). `npm audit` -> 0 vulnerabilities.
- Rust: `tauri` 2.10.2 -> **2.11.5** (Origin Confusion IPC advisory). `rand` dropped from the tree entirely by the refresh.

**Residual (expected, upstream-gated):**
- `glib` 0.18.5 — RUSTSEC medium unsoundness, capped by Tauri 2.x gtk-rs 0.18. Recheck when Tauri releases on gtk-rs 0.20.

**Key resolved versions (held on their current major on purpose):**
- typescript 5.9.3 · tailwindcss 3.4.19 · tailwind-merge 2.6.1 · next 15.5.20 · eslint 9.39.5 · eslint-config-next 15.5.20
...
All gates green: typecheck, lint, `next build`, `cargo build`.
